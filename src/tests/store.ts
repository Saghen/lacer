...
test("CounterStore actions", (t) => {
  ...
  const increment = () =>
    CounterStore.set((prev) => ({ count: prev.count + 1 }), "Increment");
  const decrement = () =>
    CounterStore.set((prev) => ({ count: prev.count - 1 }), "Decrement");
...
test("CounterStore actions with condition", (t) => {
  ...
  const decrement = () =>
    CounterStore.set((prev) => ({ count: prev.count - 1 }), "Decrement");
  const sudoDecrement = () =>
    CounterStore.set((prev) => ({ count: prev.count - 1 }), "SudoDecrement");
...
test("TestStore actions", (t) => {
  ...
  // Implementing some actions to update the store
  const decrement = () =>
    TestStore.set((prev) => ({ count: prev.count - 1 }), "Decrement");
  const toggle = () =>
    TestStore.set((prev) => ({ toggle: !prev.toggle }), "Toggle");
...
test("Global state", (t) => {
  ...
  t.assert(
    JSON.stringify(getGlobalState()) === JSON.stringify(getGlobalState())
  );
